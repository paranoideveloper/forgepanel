...
The goal is feature parity with BPB-Worker-Panel / Nova-Proxy, with the install
doing all the work. Batch 1:
...
Running a real zone (`s13.example.net`, masterdns) on the Docker host surfaced
...
s13→ns.example.net→203.0.113.16, and `:53` now answers tunnel-style queries.
