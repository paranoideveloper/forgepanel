...
docker run -d --name forgepanel --restart unless-stopped \
  ...
  ghcr.io/paranoideveloper/forgepanel:v1.8.0
```
...
```bash
docker image inspect forgepanel:v1.8.0
docker run --rm --entrypoint /usr/local/bin/forgectl forgepanel:v1.8.0 version
```
