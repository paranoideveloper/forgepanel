...
func TestStore_RoundTripAndEncryption(t *testing.T) {
	...
	if out, _ := s.EnsureRequest(222, "meti", "Meti"); out != RequestNew {
		t.Fatalf("first request outcome = %v", out)
	}
	if out, _ := s.EnsureRequest(222, "meti", "Meti"); out != RequestPending {
	...
	if containsBytes(blob, "cf-secret-token") || containsBytes(blob, "meti") {
