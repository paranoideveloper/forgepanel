...
func awgGenNode(gen string) *model.Node {
	n := &model.Node{
		Protocol: model.ProtoAmneziaWG, Address: "51.91.79.136", Port: 51851,
...
func TestAWGGenerationEmitsOnlyItsOwnKeys(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.gen, func(t *testing.T) {
			conf, err := AmneziaWGConf(awgGenNode(tc.gen), "51.91.79.136")
			if err != nil {
...
// verified against a real AmneziaWG 3.1 kernel module (s7, 51.91.79.136:51851 —
...
func TestAWG31MatchesAMeasuredWorkingServer(t *testing.T) {
	conf, err := AmneziaWGConf(awgGenNode(model.AWG31), "51.91.79.136")
	if err != nil {
		t.Fatal(err)
	}
