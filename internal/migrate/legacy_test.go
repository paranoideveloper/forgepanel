package migrate
import ("testing"
 "github.com/glebarez/sqlite";"gorm.io/gorm";"gorm.io/gorm/logger"
...
func TestImportPanelDB(t *testing.T){
 ...
 // a VLESS-REALITY inbound with two clients (mirrors the real yon.ink an existing panel box)
 ...
 res,err:=ImportPanelDB(dir); if err!=nil{t.Fatal(err)}
 if len(res.Inbounds)!=2{t.Fatalf("expected 2 inbounds, got %d (%v)",len(res.Inbounds),res.Warnings)}
 // vless reality inbound
 var vless *ImportedInbound
 for i:=range res.Inbounds{ if res.Inbounds[i].Node.Protocol==model.ProtoVLESS{vless=&res.Inbounds[i]} }
 if vless==nil{t.Fatal("vless inbound missing")}
 if vless.Node.Security.Type!=model.SecReality{t.Fatal("reality not mapped")}
 if len(vless.Users)!=2{t.Fatalf("expected 2 users, got %d",len(vless.Users))}
 if vless.Users[0].UUID!="11111111-2222-3333-4444-555555555555"{t.Fatal("client uuid not lifted")}
}
