...
func ValidateURL(p Policy, raw string) (*url.URL, error) {
	...
	// user:password in a fetch target is never legitimate here and is a way to
	// get a credential logged, echoed back in an error, or sent to the wrong
	// host after a redirect.
	if u.User != nil {
		return nil, fmt.Errorf("%q carries embedded credentials; put them in a header instead", raw)
	}
