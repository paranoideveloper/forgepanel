...
type SingboxStatsSupport struct {
...
// question.
func detectSingboxStats(bin string) SingboxStatsSupport {
	if strings.TrimSpace(bin) == "" {
		return SingboxStatsSupport{Reason: "sing-box is not installed"}
	}
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	out, err := exec.CommandContext(ctx, bin, "version").CombinedOutput()
	if err != nil {
		return SingboxStatsSupport{Reason: "could not run sing-box: " + err.Error()}
	}
	sup := SingboxStatsSupport{}
...
func (c *Controller) SingboxStatsSupported() SingboxStatsSupport {
	c.sbStatsOnce.Do(func() {
		c.sbStats = detectSingboxStats(c.bins.Path(binmgr.EngineSingbox))
	})
	return c.sbStats
}
