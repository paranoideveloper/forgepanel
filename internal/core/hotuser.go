...
func (c *Controller) xrayHotApply(oldCfg, newCfg []byte) (bool, error) {
	...
	for _, d := range deltas {
		if len(d.Remove) > 0 {
			if err := removeXrayUsers(bin, server, d.Tag, d.Remove); err != nil {
				return false, err
			}
		}
		if len(d.Add) > 0 {
			if err := addXrayUsers(bin, server, c.hotApplyDir(), d); err != nil {
				return false, err
			}
		}
	}
...
func addXrayUsers(bin, server, dir string, d userDelta) error {
...
func removeXrayUsers(bin, server, tag string, emails []string) error {
...
// hotApplyDir is where the short-lived `adu` documents are written. Under the
// panel's own data directory rather than the system temp dir: they carry user
// credentials, and a world-readable /tmp on a shared host would expose them for
